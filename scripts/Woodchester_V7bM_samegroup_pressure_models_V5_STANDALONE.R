# =============================================================================
# WOODCHESTER V7b-M SAME-SOCIAL-GROUP INFECTION-PRESSURE SENSITIVITY — V5
# STANDALONE NUMERIC-MATRIX IMPLEMENTATION
#
# This is a complete standalone implementation. It deliberately does NOT patch,
# rewrite or eval() an older script. The earlier V2-V4 development versions are
...
#
# Pressure is same-social-group prevalence at Q4 of movement-ending year t.
# Outcome is infection acquisition in Q1-Q4 of t+1.
# Pressure is scaled so 1 coefficient unit = +10 percentage points prevalence.
#
# M0_FULL -> M0_CC diagnoses pressure-support sample restriction.
# M0_CC -> M1_RAW/M1_SMOOTH diagnoses pressure adjustment on identical rows.
# M1/M2 are mechanistic/direct-effect sensitivities, not replacements for the
# frozen primary V7b-M total-effect analysis.
# =============================================================================
...
for(f in c(MOVE_FILE,PAIR_FILE,INF_FILE,PRESS_FILE)) if(!file.exists(f)) stop("Missing required file: ",f)
...
RESULT_TAG <- Sys.getenv("RESULT_TAG","FULL_1500_V5")
set.seed(SEED)

START_YEAR <- as.integer(inf$start_year)
pidx <- as_tibble(press$interval_index) %>%
  mutate(pressure_row=row_number(),tattoo=trimws(as.character(tattoo)),model_i=as.integer(model_i),
         interval_col=as.integer(interval_col),pressure_year=as.integer(pressure_year),
...
if(!all(dim(P_RAW)==c(nrow(pidx),ncol(inf$infection_time)))) stop("Raw pressure matrix dimension mismatch.")
...
rmvn_psd <- function(n,mu,Sigma){
  mu <- as.numeric(mu)
  if(!length(mu) || any(!is.finite(mu))) stop("MVN mean is empty or non-finite.")
  Sigma <- as.matrix(Sigma); Sigma <- (Sigma+t(Sigma))/2
  if(!all(dim(Sigma)==c(length(mu),length(mu))) || any(!is.finite(Sigma))) stop("MVN covariance invalid.")
  ...
  for(j in rr){
    ...
    for(qtime in start_q[j]:last_q){
      ...
      out[[oo]] <- tibble(tattoo=pidx$tattoo[j],sex=as.integer(mov$sex[pidx$model_i[j]]),movement_state=st[j],
                          pressure=pressure[j],pressure_smooth=pressure_smooth[j],group_n=GROUP_N[j],
...
build_X <- function(d,model){
  ...
  if(model=="M1_RAW" || model=="M2_INTERACTION") cols$pressure10 <- 10*as.numeric(d$pressure)
...
fit_one <- function(model,d,nkeep){
  ...
  cluster <- trimws(as.character(d$tattoo))
  X <- build_X(d,model)

  finite_by_col <- colSums(!is.finite(X))
  bad_y <- sum(!is.finite(y)); bad_cluster <- sum(is.na(cluster) | !nzchar(cluster))
  if(any(finite_by_col>0) || bad_y>0 || bad_cluster>0){
    ...
    return(list(ok=FALSE,draws=NULL,converged=FALSE,warning="",
                message=paste0("non-finite inputs; X[",bad_cols,"]; y=",bad_y,"; cluster=",bad_cluster)))
  }
  if(!length(y) || !all(y%in%c(0,1))) return(list(ok=FALSE,draws=NULL,converged=FALSE,warning="",message="invalid binary outcome"))
  ...
  if(length(mu)!=length(y) || !length(mu) || any(!is.finite(mu))) return(list(ok=FALSE,draws=NULL,converged=TRUE,warning=paste(unique(warns),collapse=" | "),message="invalid fitted probabilities"))
  ...
  if(inherits(rob,"error")) return(list(ok=FALSE,draws=NULL,converged=TRUE,warning=paste(unique(warns),collapse=" | "),message=paste("robust covariance failed:",conditionMessage(rob))))
  if(any(!is.finite(rob$V))) return(list(ok=FALSE,draws=NULL,converged=TRUE,warning=paste(unique(warns),collapse=" | "),message="non-finite robust covariance"))
...
cat("\n============================================================\n")
cat("V7b-M SAME-GROUP PRESSURE SENSITIVITY — V5 STANDALONE\n")
cat("============================================================\n")
...
for(pp in seq_len(nrow(pair_index))){
  ...
  d_full <- make_risk_data(move_draw,inf_col,FALSE)
  d_cc <- make_risk_data(move_draw,inf_col,TRUE)
  ...
  for(nm in names(fits)){
    ...
    diag_list[[ng]] <- tibble(pair_draw=pair_index$pair_draw[pp],movement_draw=move_draw,infection_col=inf_col,model=nm,
                              ...
                              n_high_rows=sum(dd$movement_state==1L),fit_ok=z$ok,converged=z$converged,
                              warnings=z$warning,message=z$message)
    if(z$ok){dr <- z$draws; dr$pair_draw <- pair_index$pair_draw[pp]; dr$model <- nm; nd <- nd+1L; draw_list[[nd]] <- dr}
  }
...
if(any(!diag$fit_ok)){
  cat("\nFailures:\n")
  print(diag %>% filter(!fit_ok) %>% count(model,message,sort=TRUE),n=Inf,width=Inf)
}
...
for(nm in unique(draws$model)){
  ...
  if("movement_state:pressure10"%in%names(z)){ns <- ns+1L; summary_rows[[ns]] <- summarise_parameter(z,"movement_state:pressure10","beta_move_x_pressure_per_10pp")}
...
if("M2_INTERACTION"%in%draws$model){
  ...
  if(all(c("movement_state","movement_state:pressure10")%in%names(z))){
    interaction_derived <- bind_rows(lapply(c(0,.10,.25,.50),function(p){
      ...
      tibble(pressure_prevalence=p,OR_move_median=median(exp(lp)),OR_move_q025=unname(quantile(exp(lp),.025)),
             OR_move_q975=unname(quantile(exp(lp),.975)),P_move_positive=mean(lp>0))
    }))
...
saveRDS(list(model="V7b-M same-social-group pressure mechanistic sensitivity V5 standalone",coefficient_draws=draws,
             parameter_summary=param_summary,movement_comparison=movement_compare,interaction_derived=interaction_derived,
             fit_audit=fit_audit,fit_diagnostics=diag,settings=list(min_group_n=MIN_GROUP_N,paired_histories=nrow(pair_index),
             draws_per_history=N_KEEP,pressure_time="Q4(t)",outcome_time="Q1-Q4(t+1)",pressure_scale="10 percentage points",
             covariance="badger-cluster robust",complete_case_benchmark=TRUE)),out_rds)
