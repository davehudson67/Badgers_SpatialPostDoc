# =============================================================================
# WOODCHESTER V7a-M WITHIN-INDIVIDUAL SENSITIVITY v2
#
# FIXES / IMPROVEMENTS
# --------------------
# 1. Attaches survival explicitly so strata() is available when clogit evaluates
#    the formula.
# 2. Removes outcome-uninformative badger strata before fitting.
# 3. Prints the within-individual information audit BEFORE fitting.
# 4. Records and prints actual fit errors rather than ending with an opaque
#    "no fits estimable" message.
#
# QUESTION
# --------
...
# lag 0 = infected by Q4 origin year
# lag 1 = infected by Q4 previous year
infection_cutoff <- 4L*((idx$from_year-INF_LAG_YEARS)-START_YEAR)+4L

# One fixed calendar-time basis used in every latent-history imputation.
YB <- splines::ns(
  idx$from_year,
  df=5,
...
cat("\n============================================================\n")
cat("V7a-M WITHIN-INDIVIDUAL SENSITIVITY v2\n")
cat("============================================================\n")
...
for(pp in seq_len(nrow(pair_index))){
  ...
  # V7a remains specifically an R -> high initiation model.
  ...
  # Which individuals can contribute to an individual-FE binary outcome model?
  id_info <- d %>%
    group_by(tattoo) %>%
    summarise(
      n=n(),
  ...
  # Print an early audit for the first latent history.
  if(pp==1L){
  ...
  fit <- tryCatch(
    ...
    error=function(e)e,
    warning=function(w){
      # Preserve warnings as messages only if fit otherwise succeeds.
  ...
  # Conditional-likelihood normal approximation for this sensitivity analysis.
...
if(!nrow(pooled)){
  stop(
    ...
    "The failure messages printed immediately above identify whether this is ",
    "a data-information issue or a model/formula issue."
  )
}
...
out_rds <- paste0(
  "results/V7aM_within_individual_v2_lag",
...
saveRDS(
  list(
    model="V7a-M within-individual conditional logistic sensitivity v2",
