...
# Function to find the last capture occasion
find_last <- function(row) {
  # Find the last column index where capture (1) occurred
  last_capture_index <- max(which(row == 1))
  return(last_capture_index)
}

last_capture <- apply(capture_history, 1, find_last)
...
#G <- G[1:10,,]
#first_capture <- first_capture[1:10]

data <- list(Y = capture_history,  G = G)
consts <- list(nind = nrow(capture_history), f = first_capture, n.occasions = ncol(capture_history), l = last_capture, 
