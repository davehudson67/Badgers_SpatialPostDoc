# =============================================================================
# WOODCHESTER SPATIAL CMR V7b-T
# HIGH MOBILITY -> SUBSEQUENT INFECTION ACQUISITION
#
# V6c movement scaffold with one coherent sampled infection trajectory.
#
# Directional definition:
#   disp[i,k] = movement state for annual AC change year k-1 -> year k.
#   That completed movement state predicts infection acquisition during
#   Q1-Q4 of the FOLLOWING calendar year (year k+1).
#
# Only badger-quarters still susceptible at Q4 of year k enter the risk set.
# Infection is absorbing. A sampled infection event ends that trajectory's
# contribution to later susceptible risk periods.
#
# Quarterly discrete-time acquisition model:
#   logit(hazard) =
#     alpha_acq +
#     beta_acq_move * high_mobility_previous_year +
#     beta_acq_sex * male +
#     quarter effect +
#     5-year calendar-period effect
#
# PRIMARY PARAMETER:
#   beta_acq_move; OR_acq_high = exp(beta_acq_move)
#
# This is deliberately a strict temporal lag: completed mobility precedes the
# future infection-risk year. It is therefore more conservative than using
# movement and infection from the same year.
#
# Default is a short pilot. Set PILOT=false for the production fit and use
# TRAJ_DRAW to select a sampled infection trajectory.
# =============================================================================
# =============================================================================
...
cat("\n============================================================\n")
cat("WOODCHESTER V7b-T: HIGH MOBILITY -> SUBSEQUENT INFECTION\n")
cat("============================================================\n")
...
# ---- sampled infection trajectory + STRICTLY FUTURE risk set -----------------
...
# Binary Q4 status retained for audits only.
...
# Exact first/last live quarter within each selected animal's history.
live_time_bounds <- live %>%
  mutate(time=4L*(primary_year-START_YEAR)+trap_season) %>%
  group_by(tattoo) %>%
  summarise(first_live_time=min(time),last_live_time=max(time),.groups="drop") %>%
  right_join(tibble(tattoo=ids),by="tattoo") %>%
  arrange(match(tattoo,ids))

if(anyNA(live_time_bounds$first_live_time) || anyNA(live_time_bounds$last_live_time))
  stop("Could not determine first/last live quarter for all selected badgers.")

# Build quarterly infection-risk rows.
#
# Exposure:
#   disp[i,k] = movement from years[k-1] -> years[k]
#
# Outcome:
#   infection acquisition in Q1-Q4 of years[k+1]
#
# This prevents overlap between the movement interval and the infection-risk
# window and gives the direction "mobility -> subsequent infection".

risk_rows <- list()

for(i in seq_len(nind)){

  if(K[i]-first[i] < 2L) next

  inf_t <- infection_time[i]
  last_t <- live_time_bounds$last_live_time[i]

  for(move_k in (first[i]+1L):(K[i]-1L)){

    movement_end_year <- years[move_k]
    movement_end_q4_time <- 4L*(movement_end_year-START_YEAR)+4L

    # Must still be susceptible at the end of the movement year.
    susceptible_at_q4 <- inf_t==0L || inf_t>movement_end_q4_time
    if(!susceptible_at_q4) next

    outcome_year <- years[move_k+1L]
    outcome_primary <- move_k+1L

    for(q in 1:4){

      outcome_time <- 4L*(outcome_year-START_YEAR)+q
      if(outcome_time>last_t) break

      # If infection has already happened before this quarter, no longer at risk.
      if(inf_t>0L && inf_t<outcome_time) break

      event <- as.integer(inf_t>0L && inf_t==outcome_time)

      risk_rows[[length(risk_rows)+1L]] <- tibble(
        risk_i=i,
        individual_id=individual_ids[i],
        tattoo=ids[i],
        move_k=move_k,
        movement_from_year=years[move_k-1L],
        movement_to_year=years[move_k],
        outcome_year=outcome_year,
        outcome_quarter=q,
        outcome_time=outcome_time,
        period=period_vec[outcome_primary],
...
infection_risk <- bind_rows(risk_rows)
if(!nrow(infection_risk)) stop("No infection-risk rows were constructed.")

n_inf_risk <- nrow(infection_risk)
risk_i <- as.integer(infection_risk$risk_i)
risk_move_k <- as.integer(infection_risk$move_k)
risk_quarter <- as.integer(infection_risk$outcome_quarter)
risk_period <- as.integer(infection_risk$period)
infection_event <- as.integer(infection_risk$infection_event)

cat("\n============================================================\n")
cat("V7b-T STRICT FUTURE INFECTION RISK SET\n")
cat("============================================================\n")
cat("Trajectory draw:",TRAJ_DRAW,"\n")
cat("Risk quarters:",n_inf_risk,"\n")
cat("Sampled infection events in risk set:",sum(infection_event),"\n")
cat("Badgers contributing risk:",n_distinct(infection_risk$individual_id),"\n")
cat("Badgers with an event:",n_distinct(infection_risk$individual_id[infection_event==1L]),"\n")
cat("Movement exposure years represented:",
    min(infection_risk$movement_to_year),"to",max(infection_risk$movement_to_year),"\n")
cat("Outcome years represented:",
    min(infection_risk$outcome_year),"to",max(infection_risk$outcome_year),"\n")

if(sum(infection_event)<10L)
  warning("Very few sampled infection events in this trajectory/risk definition.")
...
code_V7 <- nimbleCode({
  ...
  # V6c movement-state transitions: NO disease effect here in V7b-T.
  ...
  # ---------------------------------------------------------------------------
  # Future quarterly infection-acquisition hazard
  # ---------------------------------------------------------------------------
  alpha_acq ~ dnorm(qlogis(.02),sd=1.5)
  beta_acq_move ~ dnorm(0,sd=1)   # PRIMARY V7b-T PARAMETER
  beta_acq_sex ~ dnorm(0,sd=1)

  for(q in 1:3){
    beta_acq_quarter_raw[q] ~ dnorm(0,sd=.75)
    beta_acq_quarter[q] <- beta_acq_quarter_raw[q]
  }
  beta_acq_quarter[4] <- -sum(beta_acq_quarter_raw[1:3])

  for(p in 1:(n_periods-1)){
    beta_acq_period_raw[p] ~ dnorm(0,sd=.75)
    beta_acq_period[p] <- beta_acq_period_raw[p]
  }
  beta_acq_period[n_periods] <- -sum(beta_acq_period_raw[1:(n_periods-1)])

  OR_acq_high <- exp(beta_acq_move)
  OR_acq_male <- exp(beta_acq_sex)

  # Typical (sum-to-zero time effects = 0) quarterly hazards.
  p_acq_female_local <- ilogit(alpha_acq)
  p_acq_female_high <- ilogit(alpha_acq+beta_acq_move)
  p_acq_male_local <- ilogit(alpha_acq+beta_acq_sex)
  p_acq_male_high <- ilogit(alpha_acq+beta_acq_sex+beta_acq_move)
  ...
  for(i in 1:nind){
    ...
    for(k in (first[i]+1):K[i]){
      ...
      # V6c movement transition, deliberately independent of infection.
      p_RD_i[i,k] <- ilogit(alpha_RD+beta_RD_sex*sex[i])
  ...
  # ---------------------------------------------------------------------------
  # STRICTLY FUTURE INFECTION ACQUISITION LIKELIHOOD
  # ---------------------------------------------------------------------------
  for(r in 1:n_inf_risk){
    lp_acq[r] <-
      alpha_acq+
      beta_acq_move*disp[risk_i[r],risk_move_k[r]]+
      beta_acq_sex*sex[risk_i[r]]+
      beta_acq_quarter[risk_quarter[r]]+
      beta_acq_period[risk_period[r]]

    p_acq[r] <- ilogit(lp_acq[r])
    infection_event[r] ~ dbern(p_acq[r])
  }
})
...
consts <- list(nind=nind,R=R,J=J,first=as.integer(first),K=as.integer(K),X=X,H=H,
               ...
               n_inf_risk=n_inf_risk,risk_i=risk_i,risk_move_k=risk_move_k,
               risk_quarter=risk_quarter,risk_period=risk_period)
...
data_list <- list(Ones=array(1L,c(nind,J,n_prim)),sex=sex_data,disp=disp_data,
                  infection_event=infection_event,state_ok=matrix(1L,nind,n_prim),
...
make_inits <- function(chain){
  ...
  list(alpha_p=warm$alpha_p+rnorm(1,0,.05),beta_p_sex=warm$beta_p_sex+rnorm(1,0,.03),
       ...
       alpha_acq=qlogis(max(.001,mean(infection_event)))+rnorm(1,0,.08),
       beta_acq_move=rnorm(1,0,.08),beta_acq_sex=rnorm(1,0,.08),
       beta_acq_quarter_raw=rnorm(3,0,.03),
       beta_acq_period_raw=rnorm(n_periods-1L,0,.03),
...
if(!grepl("dmnorm",code_txt) || !grepl("beta_acq_move",code_txt) || !grepl("infection_event",code_txt))
  stop("Required V7b-T Gaussian/future-infection structure missing.")
if(grepl("beta_RD_inf",code_txt) || grepl("p_RD_marginal",code_txt))
  stop("Infection -> movement code remains in V7b-T; movement transition must be V6c-only.")
...
build_time_V7 <- system.time(
  model_V7 <- nimbleModel(code_V7,constants=consts,data=data_list,inits=inits[[1]],
    dimensions=list(Ones=dim(data_list$Ones),sex=nind,disp=c(nind,n_prim),
                    infection_event=n_inf_risk,state_ok=c(nind,n_prim),
...
core_monitors <- c(
  ...
  "p_RD_female","p_RD_male","p_DD_female","p_DD_male","OR_RD_male",
  "alpha_acq","beta_acq_move","beta_acq_sex","OR_acq_high","OR_acq_male",
  "p_acq_female_local","p_acq_female_high","p_acq_male_local","p_acq_male_high",
  "beta_acq_quarter","beta_acq_period",
...
acq_global <- c("alpha_acq","beta_acq_move","beta_acq_sex")
...
for(b in list(move_global,RD_global,DD_global,acq_global,init_global,detect_p_global,detect_sigma_global,landscape_global)){
...
diag_params <- unique(c(
  intersect(setdiff(core_monitors,c("beta_season","beta_period","beta_acq_quarter","beta_acq_period")),common_names),
  grep("^beta_season\\[",common_names,value=TRUE),
  grep("^beta_period\\[",common_names,value=TRUE),
  grep("^beta_acq_quarter\\[",common_names,value=TRUE),
  grep("^beta_acq_period\\[",common_names,value=TRUE)
))
...
beta_move_inf_draws <- unlist(lapply(samples_V7,function(ch) as.matrix(ch)[,"beta_acq_move"]))
or_move_inf_draws <- exp(beta_move_inf_draws)

infection_effect_summary <- tibble(
  quantity=c("beta_acq_move","OR_acq_high","P(beta_acq_move > 0)"),
  mean=c(mean(beta_move_inf_draws),mean(or_move_inf_draws),mean(beta_move_inf_draws>0)),
  q2.5=c(quantile(beta_move_inf_draws,.025),quantile(or_move_inf_draws,.025),NA_real_),
  median=c(median(beta_move_inf_draws),median(or_move_inf_draws),NA_real_),
  q97.5=c(quantile(beta_move_inf_draws,.975),quantile(or_move_inf_draws,.975),NA_real_)
)

cat("\n============================================================\n")
cat("PRIMARY V7b-T MOBILITY -> SUBSEQUENT INFECTION EFFECT\n")
cat("============================================================\n")
...
if(SAVE_LATENT_STATES){
  ...
  risk_posterior <- infection_risk %>%
    mutate(
      node=paste0("disp[",risk_i,", ",move_k,"]"),
      p_high_mobility=unname(posterior_means[node])
    )

  infection_state_descriptive <- risk_posterior %>%
    group_by(infection_event) %>%
    summarise(
      n=n(),
      mean_p_high=mean(p_high_mobility,na.rm=TRUE),
      median_p_high=median(p_high_mobility,na.rm=TRUE),
      .groups="drop"
    )

  print(pattern_counts,n=Inf); print(state_occupancy)
  cat("\nP(high mobility) in future-infection risk rows:\n")
  print(infection_state_descriptive,n=Inf)
} else {
...
# ---- save -------------------------------------------------------------------
result_file <- paste0(
  "results/RD_SCR_V7bT_MOBILITY_TO_INFECTION_DRAW_",TRAJ_DRAW,
...
saveRDS(list(
  ...
  infection_risk=infection_risk,risk_posterior=risk_posterior,
  ...
  settings=list(model="V7bT_high_mobility_to_subsequent_infection",
    ...
    infection_alignment="sampled infection acquisition in quarters of year after completed movement state",
    infection_uncertainty="one coherent sampled absorbing trajectory",infection_thresholded=FALSE,
    primary_parameter="beta_acq_move",
    directionality="dispersion state ending year t predicts infection acquisition in Q1-Q4 year t+1",
    infection_baseline_adjustment="sex + quarter + 5-year calendar period",
...
cat("Primary parameter: beta_acq_move; OR = exp(beta_acq_move)\n")
