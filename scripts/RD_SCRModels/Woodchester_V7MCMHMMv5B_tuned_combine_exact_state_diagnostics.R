# =============================================================================
# V7MC-MHMM v5B TUNED COMBINE + EXACT POSTERIOR STATE AGREEMENT
# =============================================================================
library(tidyverse)
library(coda)

CHAIN_FILES <- sprintf(
  "results/RD_SCR_V7MCMHMMv5B_TARGETED_AFSLICE_1285_CHAIN_%d_TUNED_15K.rds",
  1:3
)
...
classic_rhat <- function(xs){
  n<-min(vapply(xs,length,integer(1))); xs<-lapply(xs,function(x) as.numeric(x[seq_len(n)]))
...
saveRDS(list(global_diagnostics=global_diag,per_chain=per_chain,pairwise=pairwise,
             ...
             settings=list(model="V7MCMHMMv5B_targeted_AFslice",
                           latent_diagnostic="exact_forward_backward_smoothed_P_high")),
        "results/V7MCMHMMv5B_TUNED_15K_diagnostics.rds")
cat("\nSaved: results/V7MCMHMMv5B_TUNED_15K_diagnostics.rds\n")
