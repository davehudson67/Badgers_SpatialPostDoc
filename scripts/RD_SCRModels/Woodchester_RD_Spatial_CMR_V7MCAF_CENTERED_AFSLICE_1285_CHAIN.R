# =============================================================================
# WOODCHESTER V7MC-AF - CENTERED MOVEMENT MODEL + BIVARIATE AF_SLICE AC SAMPLERS
#
# Purpose
...
#   - CENTERED annual ACs + four quarterly capture occasions
...
cat("\n============================================================\n")
cat("WOODCHESTER V7MC-AF: CENTERED + BIVARIATE AF_SLICE ACTIVITY CENTRES\n")
cat("============================================================\n")
...
code_V7 <- nimbleCode({
  ...
  for(i in 1:nind){
    ...
    for(k in (first[i]+1):K[i]){
      ...
      # Centered Gaussian annual AC movement.
      # Mathematically equivalent to S_k = S_{k-1} + sigma*eps, eps~N(0,I),
      # but S_k is stochastic directly so MCMC updates have local dependencies.
      S[i,1,k] ~ dnorm(mean=S[i,1,k-1],sd=sigma_move[i,k])
      S[i,2,k] ~ dnorm(mean=S[i,2,k-1],sd=sigma_move[i,k])
      moveDist[i,k] <- sqrt(pow(S[i,1,k]-S[i,1,k-1],2)+
  ...
  # Packed deterministic copy of only the biologically modeled movement states.
  # This is monitored instead of the sparse rectangular disp array.
  for(r in 1:n_disp_save)
    disp_save[r] <- disp[disp_save_i[r],disp_save_k[r]]
})
...
consts <- list(nind=nind,R=R,J=J,first=as.integer(first),K=as.integer(K),X=X,H=H,
               ...
               n_disp_save=n_disp_save,disp_save_i=disp_save_i,disp_save_k=disp_save_k)
...
make_inits <- function(chain){
  ...
  for(i in seq_len(nind)){
    ...
    for(k in (first[i]+1L):K[i]){
      ...
      dd <- sqrt(dx^2+dy^2)
      disp0[i,k] <- rbinom(1,1,plogis((dd-150)/50))
    }
  }
...
if(grepl("dmvt",code_txt))
  stop("Student-t movement found: V7M-C must use Gaussian movement.")
if(grepl("eps\\[",code_txt))
  stop("Non-centered eps nodes remain in centered model.")
if(!grepl("S\\[i, 1, k\\] ~ dnorm|S\\[i,1,k\\] ~ dnorm",code_txt))
  warning("Could not text-match centered S stochastic statement; inspect code if build fails.")
...
cat("\nCentered model code checks: PASS\n")
...
# `disp_save` has exactly n_disp_save = 5,667 cells, rather than the full
# sparse rectangular disp array that produced >64k saved columns previously.
monitors <- c(core_monitors,"disp_save")
...
cat("Packed movement-state nodes saved:",n_disp_save,"\n")
...
result_file <- sprintf(
  "results/RD_SCR_V7MCAF_CENTERED_AFSLICE_1285_CHAIN_%d%s.rds",
  CHAIN_ID,RESULT_TAG
)
...
saveRDS(
  list(
    ...
    settings=list(
      model="V7MCAF_centered_bivariate_AFslice_movement_only",
      ...
      parameterisation="centered_stochastic_activity_centres",
