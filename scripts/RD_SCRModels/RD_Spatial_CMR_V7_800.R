# =============================================================================
# WOODCHESTER SPATIAL CMR V7a
# INFECTION -> FUTURE HIGH-MOBILITY / RELOCATION
#
# V6c scaffold:
#   - annual ACs, four quarterly capture occasions
#   - two-state Gaussian AC movement: stable/local vs high-mobility
#   - sex effects on movement magnitude and state transitions
#   - normalized SG/peripheral resistance
#   - movement from first live year to last observed live year
#   - >=2 observed live years, 2026 excluded, survival conditioned out
#
# V7a extension:
#   Q4 "all_tests_inferred" P(infected) in origin year t predicts the
#   R->D transition for the next annual movement interval.
#
# Infection is NOT thresholded. If q=P(I=1):
#   p_RD = (1-q)*P(R->D|I=0) + q*P(R->D|I=1)
# so beta_RD_inf remains the log-odds contrast infected vs uninfected.
# =============================================================================
...
infection_file <- "data/badger_infection_quarterly_all_tests_inferred_20260827.rds"
sett_file <- "data/WoodchesterSettLocations.csv"
spatial_file <- "data/spatial/V3_spatial_inputs_50m_2km.rds"

required_files <- c(encounter_file,individual_file,infection_file,sett_file,spatial_file)
...
cat("\n============================================================\n")
cat("WOODCHESTER V7a: INFECTION -> FUTURE HIGH MOBILITY\n")
cat("============================================================\n")
...
infection_obj <- readRDS(infection_file)
infection_q <- if(is.list(infection_obj) && "data" %in% names(infection_obj)) infection_obj$data else infection_obj
...
# ---- infection quarter grid -> Q4 origin-year matrix -------------------------
infection_q <- infection_q %>%
  mutate(individual_id=as.integer(individual_id),year=as.integer(year),quarter=as.integer(quarter),
         infection_probability=as.numeric(infection_probability)) %>%
  filter(individual_id %in% individual_ids,year<=MAX_YEAR)
if(nrow(infection_q %>% count(individual_id,year,quarter) %>% filter(n>1))) stop("Duplicate infection quarters.")

p_inf_q <- array(NA_real_,dim=c(nind,n_prim,4L))
for(rr in seq_len(nrow(infection_q))){
  i <- match(infection_q$individual_id[rr],individual_ids)
  k <- match(infection_q$year[rr],years); q <- infection_q$quarter[rr]
  if(!is.na(i) && !is.na(k) && q %in% 1:4) p_inf_q[i,k,q] <- infection_q$infection_probability[rr]
}
p_inf_q4 <- p_inf_q[,,4L,drop=TRUE]
p_inf_mean <- apply(p_inf_q,c(1,2),function(x) if(all(is.na(x))) NA_real_ else mean(x,na.rm=TRUE))
  ...
  if(K[i]-first[i]>=2L){
    for(k in (first[i]+2L):K[i]){
      transition_rows[[length(transition_rows)+1L]] <- tibble(
        ...
        infection_origin_year=years[k-1L],p_infected_q4=p_inf_q4[i,k-1L])
    }
  }
}
transition_audit <- bind_rows(transition_rows)
cat("Movement-state transitions informing beta_RD_inf:",nrow(transition_audit),"\n")
cat("Missing Q4 infection values:",sum(is.na(transition_audit$p_infected_q4)),"\n")
if(anyNA(transition_audit$p_infected_q4)) stop("Missing Q4 infection probability for V7 transition.")

# NIMBLE evaluates p_RD for the first interval too, so require/fill all origin cells.
for(i in seq_len(nind)){
  origin <- first[i]:(K[i]-1L)
  if(anyNA(p_inf_q4[i,origin])) stop("Missing Q4 infection inside modeled history for ",ids[i])
}
p_inf_q4_model <- p_inf_q4
p_inf_q4_model[is.na(p_inf_q4_model)] <- 0
...
code_V7 <- nimbleCode({
  ...
  for(i in 1:nind){
    ...
    for(k in (first[i]+1):K[i]){
      ...
      # Marginalize over probabilistic infection at Q4 in origin year k-1.
      p_RD0[i,k] <- ilogit(alpha_RD+beta_RD_sex*sex[i])
      p_RD1[i,k] <- ilogit(alpha_RD+beta_RD_sex*sex[i]+beta_RD_inf)
      p_RD_marginal[i,k] <- (1-p_inf_q4[i,k-1])*p_RD0[i,k]+p_inf_q4[i,k-1]*p_RD1[i,k]
      p_DD_i[i,k] <- ilogit(alpha_DD+beta_DD_sex*sex[i])
      p_disp_markov[i,k] <- (1-disp[i,k-1])*p_RD_marginal[i,k]+disp[i,k-1]*p_DD_i[i,k]
...
data_list <- list(Ones=array(1L,c(nind,J,n_prim)),sex=sex_data,disp=disp_data,
                  p_inf_q4=p_inf_q4_model,state_ok=matrix(1L,nind,n_prim),
...
if(!grepl("dmnorm",code_txt) || !grepl("beta_RD_inf",code_txt) || !grepl("p_RD_marginal",code_txt))
  stop("Required V7a Gaussian/infection structure missing.")
...
build_time_V7 <- system.time(
  model_V7 <- nimbleModel(code_V7,constants=consts,data=data_list,inits=inits[[1]],
                          dimensions=list(Ones=dim(data_list$Ones),sex=nind,disp=c(nind,n_prim),
                                          p_inf_q4=c(nind,n_prim),state_ok=c(nind,n_prim),
...
disp_index <- bind_rows(lapply(seq_len(nind),function(i){
  ...
  tibble(model_i=i,individual_id=individual_ids[i],tattoo=ids[i],state_k=ks,
         ...
         p_infected_q4_origin=p_inf_q4_model[i,ks-1L],
...
print(gelman.diag(samples_V7[,core_monitors],multivariate=FALSE))
print(effectiveSize(samples_V7[,core_monitors]))
...
if(SAVE_LATENT_STATES){
  ...
  infection_state_descriptive <- disp_summary %>%
    ...
    mutate(infection_bin=cut(p_infected_q4_origin,c(-Inf,.1,.5,.8,Inf),
                             labels=c("<0.1","0.1-0.5","0.5-0.8",">=0.8"))) %>%
    group_by(infection_bin) %>%
    summarise(n=n(),mean_p_infected=mean(p_infected_q4_origin),
              mean_p_high=mean(p_high_mobility),median_p_high=median(p_high_mobility),.groups="drop")
...
# ---- save -------------------------------------------------------------------
result_file <- paste0("results/RD_SCR_V7a_INFECTION_TO_HIGH_MOBILITY_",nind,"_badgers.rds")
saveRDS(list(
  ...
  infection_q4=p_inf_q4,infection_mean=p_inf_mean,transition_audit=transition_audit,
  ...
  settings=list(model="V7a_infection_to_future_high_mobility",
                ...
                encounter_file=encounter_file,individual_file=individual_file,infection_file=infection_file,
                infection_variant="all_tests_inferred",infection_alignment="Q4 P(infected) in origin year",
                infection_uncertainty="modular binary-state marginalization",infection_thresholded=FALSE,
...
# =============================================================================
# SAFE MCMC DIAGNOSTICS
# =============================================================================

# Actual parameter names present in every chain
chain_names <- lapply(
  samples_V7,
  function(x) colnames(as.matrix(x))
)
...
# Expand vector-valued monitors to their actual names
diag_params <- c(
  setdiff(
    core_monitors,
    c("beta_season","beta_period")
  ),
...
cat(
  "\nParameters available for diagnostics:",
  length(diag_params),
  "\n"
)

# Reconstruct clean mcmc.list with identical exact columns
diag_samples <- coda::mcmc.list(
  lapply(
    samples_V7,
    function(ch){
      coda::as.mcmc(
        as.matrix(ch)[,diag_params,drop=FALSE]
      )
    }
  )
)
...
gelman_results <- coda::gelman.diag(
  diag_samples,
  multivariate=FALSE,
  autoburnin=FALSE
)
...
ess_results <- coda::effectiveSize(
  diag_samples
)
...
saveRDS(
  samples_V7,
  "results/V7a_samples_RAW_backup.rds"
)
...
trace_samples <- coda::mcmc.list(
  lapply(
    samples_V7,
    function(ch){
      coda::as.mcmc(
        as.matrix(ch)[,trace_params,drop=FALSE]
      )
    }
  )
)
...
beta_inf_draws <- unlist(
  lapply(
    samples_V7,
    function(ch)
      as.matrix(ch)[,"beta_RD_inf"]
  )
)

cat(
  "P(beta_RD_inf > 0) =",
  mean(beta_inf_draws > 0),
  "\n"
)

cat(
  "P(OR infection > 1) =",
  mean(exp(beta_inf_draws) > 1),
  "\n"
)
...
p_F0 <- get_draws("p_RD_female_uninfected")
p_F1 <- get_draws("p_RD_female_infected")

p_M0 <- get_draws("p_RD_male_uninfected")
p_M1 <- get_draws("p_RD_male_infected")

rd_F <- p_F1-p_F0
rd_M <- p_M1-p_M0

risk_difference_summary <- tibble(
  comparison=c(
