...
# 400-badger NON-CENTRED AF_slice test; SG resistance temporarily removed until a
...
# ---- options ----------------------------------------------------------------
SAMPLE_N <- 400L
NITER <- 10000; NBURN <- 2500; NCHAINS <- 2
...
make_inits <- function(chain=1L){
  ...
  epsX_init <- epsY_init <- matrix(NA_real_,nind,n_prim)
  ...
  for(i in seq_len(nind)){
    ...
    if(K[i]>first[i]) for(k in (first[i]+1L):K[i]){
      epsX_init[i,k] <- (xy[k,1]-xy[k-1,1])/sigma_move_init[entry_group[i]]
      epsY_init[i,k] <- (xy[k,2]-xy[k-1,2])/sigma_move_init[entry_group[i]]
    }
  }
  ...
  list(alpha_phi=c(qlogis(.70),qlogis(.68))+rnorm(2,0,.03),
       ...
       S=S_init,epsX=epsX_init,epsY=epsY_init,z=z_init)
}
...
for(i in seq_len(nind)){
  ...
  for(k in first[i]:K[i]){
    if(k>first[i]){
      sx <- sx + sig0*inits[[1]]$epsX[i,k]
      sy <- sy + sig0*inits[[1]]$epsY[i,k]
    }
...
code_V3 <- nimbleCode({
  ...
  for(i in (N[1]+1):N[2]){
    ...
    for(k in (first[i]+1):K[i]){
      ...
      # NON-CENTRED version of exactly the same BVN/Rayleigh movement process:
      # epsX/epsY ~ N(0,1), then scale by sigma_move to obtain the annual displacement.
      epsX[i,k] ~ dnorm(0,sd=1)
      epsY[i,k] ~ dnorm(0,sd=1)
      S[i,1,k] <- S[i,1,k-1]+sigma_move[entry_group[i]]*epsX[i,k]
      S[i,2,k] <- S[i,2,k-1]+sigma_move[entry_group[i]]*epsY[i,k]
      moveDist[i,k-1] <- sigma_move[entry_group[i]]*sqrt(pow(epsX[i,k],2)+pow(epsY[i,k],2))
...
# The first AC remains stochastic and keeps NIMBLE's default samplers.
# For each subsequent year jointly update the two standard-normal innovations.
for(i in (N[1]+1L):N[2]) for(k in (first[i]+1L):K[i]){
  nodes <- c(paste0("epsX[",i,", ",k,"]"),paste0("epsY[",i,", ",k,"]"))
  config_V3$removeSamplers(nodes,print=FALSE)
  config_V3$addSampler(target=nodes,type="AF_slice", allowData = TRUE)
}
...
# ---- run ---------------------------------------------------------------------
message("\nRunning V3 NON-CENTRED development model...")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,ids=ids,detectors=detectors,det_audit=det_audit,
             settings=list(sample_n=SAMPLE_N,niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           ac_sampler="AF_slice_noncentred_epsXY",parameterisation="noncentred",sg_model="removed_pending_normalized_kernel"),
             spatial_file=spatial_file),
        paste0("results/RD_SCR_V3_habitat_noncentred_AF_",nind,"_badgers.rds"))
