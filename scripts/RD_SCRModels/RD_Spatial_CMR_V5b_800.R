# =============================================================================
# WOODCHESTER SPATIAL CMR V5b DEVELOPMENT TEST (ADULT-ENRICHED LATENT IMMIGRATION)
# 50 m habitat state space + NON-CENTRED HEAVY-TAILED Student-t3 movement
# + NORMALIZED social-group and peripheral resistance
# + SEX affecting survival, baseline detection, detection scale and movement scale.
# + LATENT IMMIGRANT STATUS for adult-entry badgers, inferred WITHOUT using sex.
# + Immigrant effects on annual movement scale and apparent survival.
# =============================================================================
...
cat("\n============================================================\n")
cat("V5b ADULT-ENRICHED LATENT IMMIGRATION MODEL\n")
cat("800 badgers | up to 400 adult-entry | Student-t3 | immigrant effects on movement/survival\n")
cat("============================================================\n\n")
...
# One baseline population now: entry-history group differences are removed.
# These are initialization / prior centres only.
SIGMA_MOVE_INIT <- 72
MOVE_PRIOR_LOGMEAN <- log(SIGMA_MOVE_INIT)

# Dynamic-normalization grid: interpolate q values linearly on log(sigma).
N_SIGMA_GRID <- 21L
...
cat("Movement estimated; initialization:",paste(round(SIGMA_MOVE_INIT,3),collapse=", "),"m\n")
cat("Dynamic normalization grid:",round(min(SIGMA_GRID),1),"to",round(max(SIGMA_GRID),1),
...
sex_lookup <- cmr_raw %>% transmute(tattoo,sex_raw=as.character(.data[[sex_col]])) %>%
  mutate(sex_clean=toupper(str_squish(sex_raw)),
         sex_code=case_when(sex_clean %in% c("F","FEMALE")~0L, sex_clean %in% c("M","MALE")~1L, TRUE~NA_integer_)) %>%
  group_by(tattoo) %>%
  summarise(sex_code={s<-sex_code[!is.na(sex_code)]; if(length(s)) as.integer(names(sort(table(s),decreasing=TRUE))[1]) else NA_integer_}, .groups="drop")
...
# Adult-enriched development sample: take as many adult-entry badgers as available,
# capped at MAX_ADULT_ENTRY, then fill the remaining SAMPLE_N slots with
# young-entry animals. Sampling within each stratum is random and reproducible
# because set.seed(123) is set above.
...
if(nrow(eligible)<SAMPLE_N) warning("Fewer eligible badgers than SAMPLE_N after stratified selection.")
...
# ---- individual metadata & death info ----------------------------------------
...
# Adult-entry is retained ONLY as evidence about possible origin.
# Cub/yearling entrants are treated as known local recruits in this first
# immigration model; adult entrants have latent immigrant status.
adult_entry <- as.integer(entry_group==2L)

# First-capture edge metric: distance from the first observed detector to the
# nearest peripheral land-cell centroid. Smaller distance = closer to the
# mapped core/peripheral boundary. Standardise the negative distance so that
# larger edge_proximity means "closer to the edge".
grid_df_edge <- sf::st_drop_geometry(sp$grid)
grid_xy_edge <- sf::st_coordinates(sp$grid)
periph_xy_edge <- grid_xy_edge[grid_df_edge$habitat==1L & grid_df_edge$zone==2L,,drop=FALSE]
if(!nrow(periph_xy_edge)) stop("No peripheral land cells found for immigrant edge metric.")

det_edge_dist <- vapply(seq_len(nrow(detectors)),function(r){
  sqrt(min((periph_xy_edge[,1]-detectors$x[r])^2+(periph_xy_edge[,2]-detectors$y[r])^2))
},numeric(1))
det_audit$edge_dist_m <- det_edge_dist

first_edge_dist_m <- det_edge_dist[first_detector]
edge_proximity <- as.numeric(scale(-first_edge_dist_m))
if(anyNA(edge_proximity) || !all(is.finite(edge_proximity))) stop("Invalid first-capture edge-proximity metric.")

# Partially observed immigrant state:
# young-entry = known local (0); adult-entry = latent (NA).
immigrant_data <- ifelse(adult_entry==0L,0L,NA_integer_)

cat("\nEntry/origin setup:\n")
cat("  young-entry known local:",sum(adult_entry==0L),"\n")
cat("  adult-entry latent origin:",sum(adult_entry==1L),"\n")
cat("  first-edge distance (m): median",round(median(first_edge_dist_m),1),
    "| range",round(min(first_edge_dist_m),1),"-",round(max(first_edge_dist_m),1),"\n")

death <- cmr %>% filter(has_pm_record,!is.na(primary)) %>% group_by(tattoo) %>%
  summarise(death_primary=min(primary),death_season={q<-trap_season[primary==min(primary)]; q<-q[!is.na(q)]; if(length(q)) min(q) else NA_integer_},.groups="drop")
...
adult_entry<-adult_entry[ord]; first_edge_dist_m<-first_edge_dist_m[ord]; edge_proximity<-edge_proximity[ord]
immigrant_data<-immigrant_data[ord]; sex_data<-sex_data[ord]; death_primary<-death_primary[ord]
...
make_inits <- function(chain=1L){
  ...
  # Initial latent sex only where genuinely unknown.
  sex_init <- sex_data
  if(length(unknown_sex_idx)) sex_init[unknown_sex_idx] <- rbinom(length(unknown_sex_idx),1,.47)
  
  # Initial immigrant states only for adult-entry animals. Sex is deliberately
  # NOT used here or in the immigrant probability model.
  immigrant_init <- rep(NA_integer_,nind)
  if(length(adult_entry_idx)){
    p0_imm_init <- plogis(qlogis(.30)+0.30*edge_proximity[adult_entry_idx])
    immigrant_init[adult_entry_idx] <- rbinom(length(adult_entry_idx),1,p0_imm_init)
  }
  
  # Initial movement scale used only to reconstruct eps from observed sett shifts.
  # Include the current sex initialization but no assumed immigrant effect.
  beta_move_sex_init <- log(1.70)
  sigma_move_init_i <- SIGMA_MOVE_INIT*exp(beta_move_sex_init*sex_init)
  ...
  for(i in seq_len(nind)){
    ...
    if(K[i]>first[i]) for(k in (first[i]+1L):K[i]){
      eps_init[i,1,k] <- (xy[k,1]-xy[k-1,1])/sigma_move_init_i[i]
      eps_init[i,2,k] <- (xy[k,2]-xy[k-1,2])/sigma_move_init_i[i]
    }
  }
  
  z_init[!is.na(z_data)] <- NA
  
  list(alpha_phi=qlogis(.81)+rnorm(1,0,.03),
       ...
       beta_phi_imm=rnorm(1,0,.08),
       beta_move_imm=rnorm(1,0,.08),
       
       psi_sex=runif(1,.43,.50),
       sex={s<-rep(NA_integer_,nind); if(length(unknown_sex_idx)) s[unknown_sex_idx]<-sex_init[unknown_sex_idx]; s},
...
# Normalization depends on current movement sigma. Precompute exact discretized
# t3 destination-class masses at log-spaced sigma knots, cache them, and
# interpolate linearly on log(sigma) during MCMC.

cat("\nPreparing dynamic t3 landscape-normalization arrays...\n")
grid_df <- sp$grid %>% sf::st_drop_geometry()
if(!all(c("row_R","col_R","SG_id","habitat","zone") %in% names(grid_df))) stop("sp$grid must contain row_R, col_R, SG_id, habitat and zone.")
grid_xy <- sf::st_coordinates(sp$grid)
land_idx <- which(grid_df$habitat==1L)
core_idx <- which(grid_df$habitat==1L & grid_df$zone==1L)
q_cache_file <- file.path("data","spatial",paste0("V4c_qgrid_t",MOVE_DF,"_",N_SIGMA_GRID,"knots_",round(min(SIGMA_GRID)),"to",round(max(SIGMA_GRID)),"m.rds"))
cache_ok <- FALSE
if(file.exists(q_cache_file)){
  qcache <- readRDS(q_cache_file)
  cache_ok <- identical(qcache$n_rows,n_rows) && identical(qcache$n_cols,n_cols) && identical(qcache$move_df,MOVE_DF) && isTRUE(all.equal(qcache$sigma_grid,SIGMA_GRID,tolerance=1e-12))
}
if(cache_ok){
  ...
  cat("No matching cache found; computing normalization grid once...\n")
  q_same_grid <- array(1,c(N_SIGMA_GRID,n_rows,n_cols)); q_other_grid <- array(0,c(N_SIGMA_GRID,n_rows,n_cols)); q_peripheral_grid <- array(0,c(N_SIGMA_GRID,n_rows,n_cols))
  calc_q_chunk <- function(origin_idx){
    ns <- length(SIGMA_GRID); out_same <- matrix(0,length(origin_idx),ns); out_other <- matrix(0,length(origin_idx),ns); out_per <- matrix(0,length(origin_idx),ns)
    for(a in seq_along(origin_idx)){
      oi <- origin_idx[a]; dx <- grid_xy[land_idx,1]-grid_xy[oi,1]; dy <- grid_xy[land_idx,2]-grid_xy[oi,2]; d2 <- dx^2+dy^2
      ...
      for(ss in seq_len(ns)){
        sig <- SIGMA_GRID[ss]; w <- (1+d2/(sig^2*(MOVE_DF-2)))^(-(MOVE_DF+2)/2); den <- sum(w)
        out_same[a,ss] <- sum(w[same])/den; out_other[a,ss] <- sum(w[other])/den; out_per[a,ss] <- sum(w[per])/den
      }
    }
    list(origin_idx=origin_idx,same=out_same,other=out_other,per=out_per)
  }
  nc_detected <- parallel::detectCores(); if(is.na(nc_detected)) nc_detected <- 1L
  ncores <- max(1L,min(8L,nc_detected-1L)); nchunks <- min(ncores,length(core_idx))
  if(nchunks==1L) chunks <- list(core_idx) else chunks <- split(core_idx,cut(seq_along(core_idx),breaks=nchunks,labels=FALSE))
  cat("Computing",length(core_idx),"core origins x",N_SIGMA_GRID,"sigma knots using",ncores,"core(s)...\n")
  q_time <- system.time({ if(.Platform$OS.type!="windows" && ncores>1L) q_parts <- parallel::mclapply(chunks,calc_q_chunk,mc.cores=ncores) else q_parts <- lapply(chunks,calc_q_chunk) })
  ...
  for(part in q_parts) for(a in seq_along(part$origin_idx)){
    ...
    q_same_grid[,rr,cc] <- part$same[a,]; q_other_grid[,rr,cc] <- part$other[a,]; q_peripheral_grid[,rr,cc] <- part$per[a,]
  }
  saveRDS(list(q_same=q_same_grid,q_other=q_other_grid,q_peripheral=q_peripheral_grid,sigma_grid=SIGMA_GRID,n_rows=n_rows,n_cols=n_cols,move_df=MOVE_DF),q_cache_file)
...
core_rc <- cbind(grid_df$row_R[core_idx],grid_df$col_R[core_idx]); max_q_error <- 0; min_q_same <- Inf
for(ss in seq_len(N_SIGMA_GRID)){
  qsum_ss <- q_same_grid[ss,,]+q_other_grid[ss,,]+q_peripheral_grid[ss,,]
  max_q_error <- max(max_q_error,max(abs(qsum_ss[core_rc]-1))); min_q_same <- min(min_q_same,min(q_same_grid[ss,,][core_rc]))
}
cat("Normalization check, max |sum(q)-1|:",max_q_error,"\n")
cat("Minimum same-SG mass across grid:",min_q_same,"; max same-SG log correction:",-log(min_q_same),"\n")
if(SOCIAL_ZERO_CONST <= -log(min_q_same)+5) stop("SOCIAL_ZERO_CONST is too small for the normalized likelihood correction.")

# ==============================================================================
# ---- OPTIMIZATION 1: CUSTOM COMPILED DETECTION LIKELIHOOD --------------------
# ==============================================================================
# This calculates the spatial hazard to all 100 traps in raw C++, 
# completely bypassing NIMBLE's graph builder to save millions of nodes.

calc_capture_prob <- nimbleFunction(
  run = function(Sx = double(0), Sy = double(0), X = double(2), 
                 sigma = double(0), lambda0_vec = double(1), 
                 H_vec = double(1), z = double(0)) {
    returnType(double(1))
    ...
    # Calculate P_alive and final season probabilities
...
code_V3 <- nimbleCode({
  ...
  # ---- one baseline population + sex + latent immigrant effects --------------
  ...
  # First immigrant extension: immigrant status can alter movement and survival,
  # but NOT detection or SG resistance. This keeps the first test identifiable.
  beta_phi_imm ~ dnorm(0,sd=.75)
  beta_move_imm ~ dnorm(0,sd=.50)
  
  # Unknown sex is inferred where needed, but sex DOES NOT inform immigrant status.
  psi_sex ~ dbeta(1,1)
  
  # Immigration model for adult-entry animals.
  # Young-entry animals have adult_entry=0, so p_imm=0 and immigrant is fixed local.
  ...
  for(i in 1:N[2]){
    ...
    logit_p_imm[i] <- alpha_imm+beta_imm_edge*edge_proximity[i]
    p_imm_adult[i] <- ilogit(logit_p_imm[i])
    p_imm[i] <- adult_entry[i]*p_imm_adult[i]
    immigrant[i] ~ dbern(p_imm[i])
    
    phi_i[i] <- ilogit(alpha_phi+beta_phi_sex*sex[i]+beta_phi_imm*immigrant[i])
    sigma_i[i] <- exp(alpha_logsigma+beta_sigma_sex*sex[i])
    
    log_sigma_move_i[i] <- alpha_logmove+beta_move_sex*sex[i]+beta_move_imm*immigrant[i]
    sigma_move_i[i] <- exp(log_sigma_move_i[i])
    
    sigma_grid_pos[i] <- (log_sigma_move_i[i]-LOG_SIGMA_MIN)/LOG_SIGMA_STEP+1
    sigma_grid_lo_raw[i] <- trunc(sigma_grid_pos[i])
    sigma_grid_lo[i] <- max(1,min(N_SIGMA_GRID-1,sigma_grid_lo_raw[i]))
    sigma_grid_frac[i] <- max(0,min(1,sigma_grid_pos[i]-sigma_grid_lo[i]))
  }
  
  # Derived survival probabilities.
  phi_female_local <- ilogit(alpha_phi)
  phi_male_local <- ilogit(alpha_phi+beta_phi_sex)
  phi_female_imm <- ilogit(alpha_phi+beta_phi_imm)
  phi_male_imm <- ilogit(alpha_phi+beta_phi_sex+beta_phi_imm)
  
  # Derived baseline detection (no immigrant effect in V5).
  ...
  # Derived movement scales for the four sex x origin combinations.
  ...
  # Keep all four possible movement scales inside the precomputed q-grid.
  move_support[1] <- step(alpha_logmove-LOG_SIGMA_MIN)*step(LOG_SIGMA_MAX-alpha_logmove)
  move_support[2] <- step(alpha_logmove+beta_move_sex-LOG_SIGMA_MIN)*
    step(LOG_SIGMA_MAX-alpha_logmove-beta_move_sex)
  move_support[3] <- step(alpha_logmove+beta_move_imm-LOG_SIGMA_MIN)*
    step(LOG_SIGMA_MAX-alpha_logmove-beta_move_imm)
  move_support[4] <- step(alpha_logmove+beta_move_sex+beta_move_imm-LOG_SIGMA_MIN)*
    step(LOG_SIGMA_MAX-alpha_logmove-beta_move_sex-beta_move_imm)
  for(mm in 1:4) move_support_ok[mm] ~ dbern(move_support[mm])
  
  # Useful derived immigrant probabilities at mean and +/-1 SD edge proximity.
  ...
  for(i in 1:N[1]){
    ...
    # Pre-calculate baseline trap prob
    ...
    # OPTIMIZED: Vectorized spatial hazard function
    captureProb[i, 1:J[i,first[i]], first[i]] <- calc_capture_prob(
      S[i,1,first[i]], S[i,2,first[i]], X[1:R, 1:2], sigma_i[i], 
      lambda0[i, 1:J[i,first[i]], first[i]], H[i, 1:J[i,first[i]], first[i]], z[i,first[i]]
    )
  ...
  for(i in (N[1]+1):N[2]){
    ...
    for(k in (first[i]+1):K[i]){
      ...
      S[i,1,k] <- S[i,1,k-1]+sigma_move_i[i]*eps[i,1,k]
      S[i,2,k] <- S[i,2,k-1]+sigma_move_i[i]*eps[i,2,k]
      moveDist[i,k-1] <- sigma_move_i[i]*sqrt(pow(eps[i,1,k],2)+pow(eps[i,2,k],2))
      ...
      in_bounds[i,k] <- step(S[i,1,k]-grid_xmin)*step(grid_xmax-S[i,1,k])*step(S[i,2,k]-grid_ymin)*step(grid_ymax-S[i,2,k])
      ...
      q_same_lo[i,k] <- q_same_grid[sigma_grid_lo[i],row_S[i,k-1],col_S[i,k-1]]
      q_same_hi[i,k] <- q_same_grid[sigma_grid_lo[i]+1,row_S[i,k-1],col_S[i,k-1]]
      q_other_lo[i,k] <- q_other_grid[sigma_grid_lo[i],row_S[i,k-1],col_S[i,k-1]]
      q_other_hi[i,k] <- q_other_grid[sigma_grid_lo[i]+1,row_S[i,k-1],col_S[i,k-1]]
      q_per_lo[i,k] <- q_peripheral_grid[sigma_grid_lo[i],row_S[i,k-1],col_S[i,k-1]]
      q_per_hi[i,k] <- q_peripheral_grid[sigma_grid_lo[i]+1,row_S[i,k-1],col_S[i,k-1]]
      q_same_here[i,k] <- q_same_lo[i,k]+sigma_grid_frac[i]*(q_same_hi[i,k]-q_same_lo[i,k])
      q_other_here[i,k] <- q_other_lo[i,k]+sigma_grid_frac[i]*(q_other_hi[i,k]-q_other_lo[i,k])
      q_per_here[i,k] <- q_per_lo[i,k]+sigma_grid_frac[i]*(q_per_hi[i,k]-q_per_lo[i,k])
      social_Z[i,k] <- q_same_here[i,k]+q_other_here[i,k]*sg_multiplier+q_per_here[i,k]*peripheral_multiplier
...
consts <- list(R=R,N=N,K=as.integer(K),J=J,first=as.integer(first),X=X,H=H,
               ...
               adult_entry=adult_entry,edge_proximity=edge_proximity,
               ...
               LOG_SIGMA_MIN=LOG_SIGMA_MIN,LOG_SIGMA_MAX=LOG_SIGMA_MAX,LOG_SIGMA_STEP=LOG_SIGMA_STEP,
               SOCIAL_ZERO_CONST=SOCIAL_ZERO_CONST)

social_zero_data <- matrix(0L,nind,n_prim)
data_list <- list(Ones=array(1L,dim(H)),z=z_data,sex=sex_data,immigrant=immigrant_data,
                  state_ok=matrix(1L,nind,n_prim),social_zero=social_zero_data,
                  move_support_ok=rep(1L,4),
                  habitat_mat=habitat_mat,SG_mat=SG_mat,zone_mat=zone_mat,
                  q_same_grid=q_same_grid,q_other_grid=q_other_grid,q_peripheral_grid=q_peripheral_grid)

# ---- model-code sanity checks ------------------------------------------------
code_txt <- paste(deparse(code_V3),collapse=" ")
if(!grepl("dmvt",code_txt)) stop("STOP: Student-t movement code not found.")
if(!grepl("immigrant\\[i\\] ~ dbern",code_txt) || !grepl("beta_move_imm",code_txt) || !grepl("beta_phi_imm",code_txt))
  stop("STOP: latent immigrant movement/survival structure not found.")
if(grepl("beta_imm_sex",code_txt)) stop("STOP: sex must NOT define immigrant status in V5.")
if(!grepl("social_Z",code_txt) || !grepl("beta_peripheral",code_txt))
  stop("STOP: normalized SG/peripheral code not found.")
if(!grepl("beta_move_sex",code_txt) || !grepl("sigma_grid_frac",code_txt))
  stop("STOP: sex-specific dynamic movement normalization not found.")
cat("\nModel code checks: PASS (t3 + sex + latent immigrant move/survival + dynamic SG normalization)\n")

message("\nBuilding V5 optimized latent-immigrant Student-t3 model...")
build_time_V3 <- system.time(
  model_V3 <- nimbleModel(code_V3,constants=consts,data=data_list,inits=inits[[1]],
                          dimensions=list(Ones=dim(H),z=dim(z_data),immigrant=nind,state_ok=c(nind,n_prim),
                                          social_zero=c(nind,n_prim),move_support_ok=4),
...
if(!is.finite(lp)) stop("V5 initial log probability is not finite.")

message("\nCompiling model...")
compile_model_time_V3 <- system.time(cModel_V3 <- compileNimble(model_V3,resetFunctions=TRUE))
cat("\nModel compile time:\n"); print(compile_model_time_V3)

monitors <- c("alpha_phi","alpha_p","alpha_logsigma","alpha_logmove",
              "beta_phi_sex","beta_p_sex","beta_sigma_sex","beta_move_sex",
              "beta_phi_imm","beta_move_imm","alpha_imm","beta_imm_edge",
              "psi_sex",
              "phi_female_local","phi_male_local","phi_female_imm","phi_male_imm",
              "p0_female","p0_male","sigma_female","sigma_male",
              "sigma_move_female_local","sigma_move_male_local",
              "sigma_move_female_imm","sigma_move_male_imm",
              "mean_move_female_local","mean_move_male_local",
              "mean_move_female_imm","mean_move_male_imm",
              "p_imm_edge_minus1","p_imm_edge_mean","p_imm_edge_plus1",
              "beta_season","beta_period","beta_sg","beta_peripheral",
              "sg_multiplier","peripheral_multiplier")
if(length(unknown_sex_idx)) monitors <- c(monitors,paste0("sex[",unknown_sex_idx,"]"))
if(length(adult_entry_idx)) monitors <- c(monitors,paste0("immigrant[",adult_entry_idx,"]"))
...
eps_nodes <- unlist(lapply((N[1]+1L):N[2],function(i){
  if(K[i] >= first[i]+1L)
    paste0("eps[",i,", 1:2, ",(first[i]+1L):K[i],"]")
  else character(0)
}),use.names=FALSE)

if(length(eps_nodes)){
  config_V3$removeSamplers(eps_nodes,print=FALSE)
  invisible(lapply(eps_nodes,function(n) config_V3$addSampler(target=n,type="AF_slice")))
}

# Joint movement block: baseline + sex + immigrant effect.
move_pars <- c("alpha_logmove","beta_move_sex","beta_move_imm")
config_V3$removeSamplers(move_pars,print=FALSE)
config_V3$addSampler(target=move_pars,type="AF_slice")

# Joint survival block: baseline + sex + immigrant effect.
surv_pars <- c("alpha_phi","beta_phi_sex","beta_phi_imm")
config_V3$removeSamplers(surv_pars,print=FALSE)
config_V3$addSampler(target=surv_pars,type="AF_slice")

# Immigration-process coefficients are naturally correlated.
imm_process_pars <- c("alpha_imm","beta_imm_edge")
config_V3$removeSamplers(imm_process_pars,print=FALSE)
config_V3$addSampler(target=imm_process_pars,type="AF_slice")
...
cat("  immigration-process block:",paste(imm_process_pars,collapse=", "),"\n")
cat("  latent adult immigrant states:",length(adult_entry_idx),"binary nodes\n")

print(config_V3)

build_mcmc_time_V3 <- system.time(Rmcmc_V3 <- buildMCMC(config_V3))
cat("\nMCMC build time:\n"); print(build_mcmc_time_V3)
message("Compiling MCMC...")
compile_mcmc_time_V3 <- system.time(cMCMC_V3 <- compileNimble(Rmcmc_V3,project=cModel_V3,resetFunctions=TRUE))
cat("\nMCMC compile time:\n"); print(compile_mcmc_time_V3)

message("\nRunning V5 800-badger LATENT IMMIGRANT + MOVE/SURVIVAL OPTIMIZED model...")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,build_time=build_time_V3,
             compile_model_time=compile_model_time_V3,build_mcmc_time=build_mcmc_time_V3,
             compile_mcmc_time=compile_mcmc_time_V3,
             ids=ids,sex_data=sex_data,entry_group=entry_group,adult_entry=adult_entry,
             immigrant_data=immigrant_data,edge_proximity=edge_proximity,
             first_edge_dist_m=first_edge_dist_m,unknown_sex_idx=unknown_sex_idx,
             adult_entry_idx=adult_entry_idx,detectors=detectors,det_audit=det_audit,
             settings=list(sample_n=SAMPLE_N,max_adult_entry=MAX_ADULT_ENTRY,n_adult_selected=sum(adult_entry==1L),n_young_selected=sum(adult_entry==0L),niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           parameterisation="noncentred",
                           movement_kernel="bivariate_Student_t",move_df=MOVE_DF,
                           sg_model="dynamic_interpolated_normalized_core_SG_plus_peripheral",
                           entry_group_effects="removed_from_survival_detection_movement",
                           immigration_model="young_entry_fixed_local_adult_entry_latent_edge_informed",
                           sex_used_to_define_immigrant=FALSE,
                           immigrant_effects=c("movement","apparent_survival"),
                           movement_global_sampler="AF_slice_alpha_logmove_beta_move_sex_beta_move_imm",
                           survival_global_sampler="AF_slice_alpha_phi_beta_phi_sex_beta_phi_imm",
                           immigrant_process_sampler="AF_slice_alpha_imm_beta_imm_edge",
                           movement_normalization="log_sigma_grid_linear_interpolation",
                           sigma_grid=SIGMA_GRID,
                           detection_implementation="custom_compiled_nimbleFunction"),
             spatial_file=spatial_file,q_cache_file=q_cache_file),
        paste0("results/RD_SCR_V5b_t",MOVE_DF,
               "_LATENT_IMMIGRANT_MOVE_SURVIVAL_OPTIMIZED_",nind,"_badgers.rds"))

MCMCsummary(samples_V3)
gelman.diag(samples_V3,multivariate=FALSE)
effectiveSize(samples_V3)

# ---- posterior immigrant probabilities & sex ratio ---------------------------
smat <- as.matrix(samples_V3)

if(length(adult_entry_idx)){
  imm_cols <- paste0("immigrant[",adult_entry_idx,"]")
  imm_keep <- imm_cols[imm_cols %in% colnames(smat)]
  imm_idx <- adult_entry_idx[match(imm_keep,imm_cols)]
  
  immigrant_posterior <- tibble(
    i=imm_idx,
    tattoo=ids[imm_idx],
    sex_code=sex_data[imm_idx],
    first_edge_dist_m=first_edge_dist_m[imm_idx],
    edge_proximity=edge_proximity[imm_idx],
    p_immigrant=colMeans(smat[,imm_keep,drop=FALSE])
  ) %>% arrange(desc(p_immigrant))
  
  cat("\nPosterior P(immigrant) for adult-entry badgers:\n")
  print(immigrant_posterior,n=Inf)
  
  # Build sex draws for all individuals. Known sex is fixed; unknown sex comes
  # from its monitored posterior. Sex was NOT used in the immigration classifier.
  sex_draw <- matrix(rep(sex_data,each=nrow(smat)),nrow=nrow(smat),ncol=nind)
  if(length(unknown_sex_idx)){
    for(ii in unknown_sex_idx){
      sc <- paste0("sex[",ii,"]")
      if(sc %in% colnames(smat)) sex_draw[,ii] <- smat[,sc]
    }
  }
  
  imm_draw <- matrix(0,nrow=nrow(smat),ncol=nind)
  imm_draw[,imm_idx] <- smat[,imm_keep,drop=FALSE]
  local_draw <- 1-imm_draw
  
  n_imm_draw <- rowSums(imm_draw)
  n_local_draw <- rowSums(local_draw)
  
  prop_male_imm <- ifelse(n_imm_draw>0,rowSums(imm_draw*sex_draw)/n_imm_draw,NA_real_)
  prop_male_local <- rowSums(local_draw*sex_draw)/n_local_draw
  male_bias_diff <- prop_male_imm-prop_male_local
  
  sexratio_summary <- tibble(
    quantity=c("Number of immigrants",
               "Male proportion among immigrants",
               "Male proportion among locals",
               "Immigrant minus local male proportion"),
    mean=c(mean(n_imm_draw),
           mean(prop_male_imm,na.rm=TRUE),
           mean(prop_male_local,na.rm=TRUE),
           mean(male_bias_diff,na.rm=TRUE)),
    q2.5=c(quantile(n_imm_draw,.025),
           quantile(prop_male_imm,.025,na.rm=TRUE),
           quantile(prop_male_local,.025,na.rm=TRUE),
           quantile(male_bias_diff,.025,na.rm=TRUE)),
    q50=c(quantile(n_imm_draw,.5),
          quantile(prop_male_imm,.5,na.rm=TRUE),
          quantile(prop_male_local,.5,na.rm=TRUE),
          quantile(male_bias_diff,.5,na.rm=TRUE)),
    q97.5=c(quantile(n_imm_draw,.975),
            quantile(prop_male_imm,.975,na.rm=TRUE),
            quantile(prop_male_local,.975,na.rm=TRUE),
            quantile(male_bias_diff,.975,na.rm=TRUE))
  )
  
  cat("\nPosterior immigrant sex-ratio summary (sex NOT used to classify immigrants):\n")
  print(sexratio_summary)
  
  saveRDS(list(individual=immigrant_posterior,sexratio=sexratio_summary),
          paste0("results/RD_SCR_V5b_immigrant_summary_",nind,"_badgers.rds"))
} else {
  immigrant_posterior <- tibble()
  sexratio_summary <- tibble()
  cat("\nNo adult-entry badgers in this sample: immigrant model cannot be evaluated.\n")
}
...
if(length(unknown_sex_idx)){
  ...
  cat("\nNo unknown-sex badgers in this sample.\n")
}