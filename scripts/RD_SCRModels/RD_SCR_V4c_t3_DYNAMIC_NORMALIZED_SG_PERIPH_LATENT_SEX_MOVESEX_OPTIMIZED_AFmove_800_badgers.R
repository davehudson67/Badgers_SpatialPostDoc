# =============================================================================
# WOODCHESTER SPATIAL CMR V4c DEVELOPMENT TEST (OPTIMIZED)
# 50 m habitat state space + NON-CENTRED HEAVY-TAILED Student-t3 movement
# + NORMALIZED social-group and peripheral resistance
# + PARTIALLY OBSERVED LATENT SEX affecting survival, baseline detection, detection scale AND movement scale.
# =============================================================================
...
cat("\n============================================================\n")
cat("V4c DYNAMIC NORMALIZED SOCIAL MOVEMENT + LATENT SEX + MOVE-SEX MODEL\n")
cat("800 badgers | Student-t3 | dynamic normalized SG/peripheral | latent sex incl. movement\n")
cat("============================================================\n\n")
...
# Movement now estimated. These are initialization/prior centres only.
SIGMA_MOVE_INIT <- c(82.867,81.732)
...
make_inits <- function(chain=1L){
  ...
  sigma_move_init <- SIGMA_MOVE_INIT
  for(i in seq_len(nind)){
  ...
  list(alpha_phi=c(qlogis(.70),qlogis(.68))+rnorm(2,0,.03), alpha_p=c(qlogis(.20),qlogis(.20))+rnorm(2,0,.03),
       alpha_logsigma=log(c(150,120))+rnorm(2,0,.03),
       alpha_logmove=MOVE_PRIOR_LOGMEAN+rnorm(2,0,.03),
       beta_phi_sex=rnorm(1,0,.05),beta_p_sex=rnorm(1,0,.05),
       beta_sigma_sex=rnorm(1,0,.05),beta_move_sex=rnorm(1,0,.03),psi_sex=runif(2,.4,.6),
...
code_V3 <- nimbleCode({
  ...
  for(grp in 1:2){
    ...
    alpha_logmove[grp] ~ dnorm(MOVE_PRIOR_LOGMEAN[grp],sd=.60)
    psi_sex[grp] ~ dbeta(1,1)
    phi_female[grp] <- ilogit(alpha_phi[grp]); phi_male[grp] <- ilogit(alpha_phi[grp]+beta_phi_sex)
    p0_female[grp] <- ilogit(alpha_p[grp]); p0_male[grp] <- ilogit(alpha_p[grp]+beta_p_sex)
    sigma_female[grp] <- exp(alpha_logsigma[grp]); sigma_male[grp] <- exp(alpha_logsigma[grp]+beta_sigma_sex)
    sigma_move_female[grp] <- exp(alpha_logmove[grp]); sigma_move_male[grp] <- exp(alpha_logmove[grp]+beta_move_sex)
    mean_move_female[grp] <- sigma_move_female[grp]*MOVE_MEAN_FACTOR; mean_move_male[grp] <- sigma_move_male[grp]*MOVE_MEAN_FACTOR
    move_support_female[grp] <- step(alpha_logmove[grp]-LOG_SIGMA_MIN)*step(LOG_SIGMA_MAX-alpha_logmove[grp])
    move_support_male[grp] <- step(alpha_logmove[grp]+beta_move_sex-LOG_SIGMA_MIN)*step(LOG_SIGMA_MAX-alpha_logmove[grp]-beta_move_sex)
    move_support_female_ok[grp] ~ dbern(move_support_female[grp]); move_support_male_ok[grp] ~ dbern(move_support_male[grp])
  }
  beta_phi_sex ~ dnorm(0,sd=1); beta_p_sex ~ dnorm(0,sd=1); beta_sigma_sex ~ dnorm(0,sd=.75); beta_move_sex ~ dnorm(0,sd=.50)
  for(i in 1:N[2]){
    ...
    log_sigma_move_i[i] <- alpha_logmove[entry_group[i]]+beta_move_sex*sex[i]
    sigma_move_i[i] <- exp(log_sigma_move_i[i])
    sigma_grid_pos[i] <- (log_sigma_move_i[i]-LOG_SIGMA_MIN)/LOG_SIGMA_STEP+1
    sigma_grid_lo_raw[i] <- trunc(sigma_grid_pos[i]); sigma_grid_lo[i] <- max(1,min(N_SIGMA_GRID-1,sigma_grid_lo_raw[i]))
...
consts <- list(R=R,N=N,K=as.integer(K),J=J,first=as.integer(first),X=X,H=H,
               ...
               MOVE_PRIOR_LOGMEAN=MOVE_PRIOR_LOGMEAN,N_SIGMA_GRID=N_SIGMA_GRID,LOG_SIGMA_MIN=LOG_SIGMA_MIN,LOG_SIGMA_MAX=LOG_SIGMA_MAX,LOG_SIGMA_STEP=LOG_SIGMA_STEP,
...
data_list <- list(Ones=array(1L,dim(H)),z=z_data,sex=sex_data,state_ok=matrix(1L,nind,n_prim),
                  social_zero=social_zero_data,move_support_female_ok=rep(1L,2),move_support_male_ok=rep(1L,2),
                  habitat_mat=habitat_mat,SG_mat=SG_mat,zone_mat=zone_mat,q_same_grid=q_same_grid,q_other_grid=q_other_grid,q_peripheral_grid=q_peripheral_grid)
...
if(!grepl("sex\\[i\\] ~ dbern",code_txt) || !grepl("psi_sex",code_txt)) stop("STOP: latent-sex code not found.")
if(!grepl("social_Z",code_txt) || !grepl("beta_peripheral",code_txt)) stop("STOP: normalized SG/peripheral code not found.")
if(!grepl("beta_move_sex",code_txt) || !grepl("sigma_grid_frac",code_txt)) stop("STOP: move-sex / dynamic normalization code not found.")
cat("\nModel code checks: PASS (t3 + move-sex + latent sex + dynamic normalized SG/peripheral)\n")

message("\nBuilding V4c optimized dynamic-normalized Student-t3 model...")
build_time_V3 <- system.time(
  model_V3 <- nimbleModel(code_V3,constants=consts,data=data_list,inits=inits[[1]],
                          dimensions=list(Ones=dim(H),z=dim(z_data),state_ok=c(nind,n_prim),
                                          social_zero=c(nind,n_prim),move_support_female_ok=2,move_support_male_ok=2),
...
if(!is.finite(lp)) stop("V4c initial log probability is not finite.")

message("\nCompiling model...")
compile_model_time_V3 <- system.time(cModel_V3 <- compileNimble(model_V3,resetFunctions=TRUE))
cat("\nModel compile time:\n"); print(compile_model_time_V3)

monitors <- c("alpha_phi","alpha_p","alpha_logsigma","alpha_logmove","psi_sex",
              "beta_phi_sex","beta_p_sex","beta_sigma_sex","beta_move_sex",
              "phi_female","phi_male","p0_female","p0_male","sigma_female","sigma_male",
              "sigma_move_female","sigma_move_male","mean_move_female","mean_move_male",
              "beta_season","beta_period","beta_sg","beta_peripheral","sg_multiplier","peripheral_multiplier")
...
# ---- CUSTOM SAMPLERS ----------------------------------------------------------
# ==============================================================================
# Annual bivariate movement innovations: retain AF_slice.
...
if(length(eps_nodes)){
  ...
  invisible(lapply(eps_nodes,function(n)
    config_V3$addSampler(target=n,type="AF_slice")))
}

# Global movement-scale parameters: sample jointly with AF_slice.
move_pars <- c("alpha_logmove[1]","alpha_logmove[2]","beta_move_sex")
config_V3$removeSamplers(move_pars,print=FALSE)
config_V3$addSampler(target=move_pars,type="AF_slice")

cat("\nCustom movement samplers assigned:\n")
cat("  annual eps blocks:",length(eps_nodes),"AF_slice targets\n")
cat("  global movement block:",paste(move_pars,collapse=", "),"\n")
...
message("\nRunning V4c 800-badger DYNAMIC NORMALIZED + MOVE-SEX OPTIMIZED model (AF movement block)...")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,build_time=build_time_V3,
             ...
             compile_mcmc_time=compile_mcmc_time_V3,ids=ids,sex_data=sex_data,
             unknown_sex_idx=unknown_sex_idx,detectors=detectors,det_audit=det_audit,
             settings=list(sample_n=SAMPLE_N,niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           ac_sampler="AF_slice_bivariate_t",parameterisation="noncentred",
                           movement_kernel="bivariate_Student_t",move_df=MOVE_DF,
                           sg_model="dynamic_interpolated_normalized_core_SG_plus_peripheral",
                           sex_model="partially_observed_latent_sex_phi_p0_detection_sigma_movement",
                           movement_sigma="estimated_entry_group_plus_sex",movement_normalization="log_sigma_grid_linear_interpolation",sigma_grid=SIGMA_GRID,movement_global_sampler="AF_slice_block_alpha_logmove1_alpha_logmove2_beta_move_sex",
                           detection_implementation="custom_compiled_nimbleFunction"),
             spatial_file=spatial_file,q_cache_file=q_cache_file),
        paste0("results/RD_SCR_V4c_t",MOVE_DF,"_DYNAMIC_NORMALIZED_SG_PERIPH_LATENT_SEX_MOVESEX_OPTIMIZED_AFmove_",nind,"_badgers.rds"))
