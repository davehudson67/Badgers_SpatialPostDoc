...
# ==============================================================================
# ---- OPTIMIZATION 2: VECTORIZED SAMPLER ASSIGNMENT ----
# ==============================================================================
message("\nAssigning custom AF_slice samplers (Vectorized)...")
eps_nodes <- character()
for(i in (N[1]+1L):N[2]) {
  if (K[i] >= (first[i] + 1L)) {
    eps_nodes <- c(eps_nodes, paste0("eps[", i, ", 1:2, ", (first[i]+1L):K[i], "]"))
  }
}
...
message("\nRunning V4c 800-badger DYNAMIC NORMALIZED + MOVE-SEX OPTIMIZED model...")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,build_time=build_time_V3,
             ...
             settings=list(sample_n=SAMPLE_N,niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           ...
                           movement_sigma="estimated_entry_group_plus_sex",movement_normalization="log_sigma_grid_linear_interpolation",sigma_grid=SIGMA_GRID,
                           detection_implementation="custom_compiled_nimbleFunction"),
             spatial_file=spatial_file,q_cache_file=q_cache_file),
        paste0("results/RD_SCR_V4c_t",MOVE_DF,"_DYNAMIC_NORMALIZED_SG_PERIPH_LATENT_SEX_MOVESEX_OPTIMIZED_",nind,"_badgers.rds"))
