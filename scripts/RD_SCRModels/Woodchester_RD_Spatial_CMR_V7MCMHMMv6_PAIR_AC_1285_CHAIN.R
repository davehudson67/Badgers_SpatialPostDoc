# =============================================================================
# WOODCHESTER V7MC-MHMM v6 - OVERLAPPING ADJACENT-YEAR AC BLOCKS
#
# Purpose
...
cat("\n============================================================\n")
cat("WOODCHESTER V7MC-MHMM v6: 4D ADJACENT-YEAR AC BLOCKS + MARGINALIZED HMM\n")
cat("============================================================\n")
...
# ---- overlapping adjacent-year 4-D activity-centre AF_slice samplers --------
# v4 updated each annual activity centre independently as a 2-D (X,Y) block.
# The remaining chain disagreement is concentrated in particular movement
# intervals, indicating that the bottleneck is now the ACTIVITY-CENTRE PATH:
# moving one annual AC alone can be strongly constrained by the movement
# likelihoods on both sides.
#
# v6 therefore removes the annual 2-D blocks and updates every ADJACENT PAIR:
#
#   (X_{k-1},Y_{k-1},X_k,Y_k)
#
# jointly with one AF_slice sampler.  Consecutive pair blocks overlap, so an
# interior annual AC is updated with both its previous and next movement edge.
# The statistical model, priors and marginalized HMM are unchanged.
#
# NIMBLE permits multiple samplers to operate on the same node; overlapping
# adjacent-pair blocks are therefore a standard MCMC-configuration operation.

S_scalar_nodes <- unlist(
...
message("\nReplacing annual 2-D S samplers with overlapping adjacent-year 4-D AF_slice blocks...")
config_V7$removeSamplers(S_scalar_nodes,print=FALSE)

invisible(lapply(
  S_pair_blocks,
  function(nodes)
    config_V7$addSampler(target=nodes,type="AF_slice")
))

cat("  annual AC nodes covered:",length(S_scalar_nodes),"\n")
cat("  overlapping 4-D adjacent-year blocks:",length(S_pair_blocks),"\n")
cat("  expected movement intervals:",n_disp_save,"\n")
if(length(S_pair_blocks)!=n_disp_save)
  stop("Adjacent-year S block count does not equal movement interval count.")
...
result_file <- sprintf(
  "results/RD_SCR_V7MCMHMMv6_PAIR_AC_1285_CHAIN_%d%s.rds",
  CHAIN_ID,RESULT_TAG
)
...
saveRDS(
  list(
    ...
    settings=list(
      model="V7MCMHMMv6_overlapping_adjacent_year_4D_AC_blocks_marginalized_HMM",
      ...
      ac_sampler="AF_slice_overlapping_4D_adjacent_year_activity_centres",
