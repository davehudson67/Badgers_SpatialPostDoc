# =============================================================================
# WOODCHESTER SPATIAL CMR V4a DEVELOPMENT TEST
...
# therefore use exactly the same movement scales. This is an architecture test
# for beta_sg and beta_peripheral, not the final joint movement/resistance model.
# =============================================================================
...
cat("\n============================================================\n")
cat("V4a NORMALIZED SOCIAL MOVEMENT MODEL\n")
cat("400 badgers | non-centred Student-t3 | normalized SG + peripheral resistance\n")
cat("============================================================\n\n")
...
SIGMA_MOVE_FIXED <- c(84.445,82.092)
...
message("\nBuilding V4a normalized Student-t3 social-movement model...")
...
if(!is.finite(lp)){
  ...
  stop("V4a initial log probability is not finite.")
}

message("\nCompiling model...")
cModel_V3 <- compileNimble(model_V3,resetFunctions=TRUE)

monitors <- c("phi_annual","sigma_move","mean_move","sigma","alpha_p","beta_season","beta_period","beta_sg","beta_peripheral","sg_multiplier","peripheral_multiplier")
...
# ---- run ---------------------------------------------------------------------
message("\nRunning V4a NORMALIZED Student-t3 SG/peripheral development model...")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,ids=ids,detectors=detectors,det_audit=det_audit,
             settings=list(sample_n=SAMPLE_N,niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           ac_sampler="AF_slice_bivariate_t",parameterisation="noncentred",movement_kernel="bivariate_Student_t",move_df=MOVE_DF,sg_model="normalized_core_SG_plus_peripheral",movement_sigma="fixed_for_normalization_test",sigma_move_fixed=SIGMA_MOVE_FIXED),
             spatial_file=spatial_file),
        paste0("results/RD_SCR_V4a_t",MOVE_DF,"_NORMALIZED_SG_PERIPH_SHORT_",nind,"_badgers.rds"))
