# =============================================================================
# WOODCHESTER SPATIAL CMR V5c DEVELOPMENT TEST (ADULT-ENRICHED LATENT IMMIGRATION: MOVEMENT ONLY)
...
# + Immigrant effect on annual movement scale ONLY; no immigrant survival effect in V5c.
# =============================================================================
...
cat("800 badgers | up to 400 adult-entry | Student-t3 | immigrant effect on movement only\n")
cat("============================================================\n\n")
...
code_V3 <- nimbleCode({
  ...
  # V5c immigrant extension: immigrant status alters MOVEMENT ONLY,
  # with no immigrant effect on survival, detection or SG resistance.
  ...
  # Derived survival probabilities. Survival is deliberately not immigrant-specific in V5c.
...
if(!grepl("immigrant\\[i\\] ~ dbern",code_txt) || !grepl("beta_move_imm",code_txt))
  stop("STOP: latent immigrant movement structure not found.")
if(grepl("beta_phi_imm",code_txt)) stop("STOP: immigrant survival effect must be absent in V5c.")
...
cat("\nModel code checks: PASS (t3 + sex + latent immigrant movement-only + dynamic SG normalization)\n")
...
monitors <- c("alpha_phi","alpha_p","alpha_logsigma","alpha_logmove",
              ...
              "beta_move_imm","alpha_imm","beta_imm_edge",
              "psi_sex",
              "phi_female","phi_male",
...
# Joint survival block: baseline + sex only.
surv_pars <- c("alpha_phi","beta_phi_sex")
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,build_time=build_time_V3,
             ...
             settings=list(sample_n=SAMPLE_N,max_adult_entry=MAX_ADULT_ENTRY,n_adult_selected=sum(adult_entry==1L),n_young_selected=sum(adult_entry==0L),niter=NITER,nburn=NBURN,nchains=NCHAINS,
                           ...
                           immigrant_effects="movement_only",
                           immigrant_survival_effect=FALSE,
                           movement_global_sampler="AF_slice_alpha_logmove_beta_move_sex_beta_move_imm",
                           survival_global_sampler="AF_slice_alpha_phi_beta_phi_sex",
        ...
        paste0("results/RD_SCR_V5c_t",MOVE_DF,
               "_LATENT_IMMIGRANT_MOVEMENT_ONLY_OPTIMIZED_",nind,"_badgers.rds"))
...
if(length(adult_entry_idx)){
  ...
  saveRDS(list(individual=immigrant_posterior,sexratio=sexratio_summary),
          paste0("results/RD_SCR_V5c_immigrant_summary_",nind,"_badgers.rds"))
} else {
  immigrant_posterior <- tibble()
  sexratio_summary <- tibble()
