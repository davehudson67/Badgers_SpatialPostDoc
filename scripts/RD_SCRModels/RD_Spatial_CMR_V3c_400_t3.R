...
# ---- options ----------------------------------------------------------------
SAMPLE_N <- 400L
NITER <- 12000; NBURN <- 3000; NCHAINS <- 2
...
saveRDS(list(samples=samples_V3,runtime=runtime_V3,ids=ids,detectors=detectors,det_audit=det_audit,
  ...
  paste0("results/RD_SCR_V3c_t",MOVE_DF,"_LONGTEST_habitat_noncentred_AF_",nind,"_badgers.rds"))

Out <- readRDS("results/RD_SCR_V3c_t3_LONGTEST_habitat_noncentred_AF_400_badgers.rds")
samples_V3 <- Out$samples
