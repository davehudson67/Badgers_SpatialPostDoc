...
#
# Implementation
#   The frozen 1,285-badger production source is patched in a small, explicitly
#   checked set of places at run time. If the source changes and an expected
#   patch is no longer unique, this script stops rather than silently fitting a
#   different model.
# =============================================================================
...
txt <- replace_txt_once(
  ...
  'required_files <- c(encounter_file,individual_file,sett_file,spatial_file,V6C_RESULT_FILE)',
  "required file list"
)
...
txt <- replace_txt_once(
  ...
  paste(c(
    'demog <- individuals %>%',
    '  transmute(individual_id=as.integer(individual_id),tattoo=as.character(tattoo),',
    '            age_fc=as.character(age_fc),year_fc=as.integer(year_fc)) %>%',
    '  mutate(entry_group=case_when(age_fc %in% c("Cub","Yearling")~1L,age_fc=="Adult"~2L,TRUE~NA_integer_),',
    '         birth_year=case_when(age_fc=="Cub" & !is.na(year_fc)~year_fc,',
...
txt <- replace_txt_once(
  ...
  paste(c(
    ...
    '  left_join(sex_lookup %>% select(individual_id,tattoo,sex_code),by=c("individual_id","tattoo")) %>%',
    '  inner_join(live_year_counts,by="individual_id") %>%',
    '  filter(!is.na(birth_year),n_live_years>=MIN_LIVE_YEARS) %>%',
    '  arrange(individual_id)',
    '',
    'if(nrow(eligible)!=EXPECTED_N)',
    '  stop("Expected ",EXPECTED_N," known-age movement badgers but reconstructed ",nrow(eligible),". Re-run the population audit before fitting.")',
    '',
    'ids <- eligible$tattoo',
    'individual_ids <- eligible$individual_id',
    'nind <- nrow(eligible)'
...
txt <- gsub("Directional population should have known sex for every badger.",
            "Known-age movement population should have known sex for every badger under the current audited snapshot.",txt,fixed=TRUE)
...
txt <- replace_txt_once(
  ...
  paste(c(
    'if(anyNA(first) || anyNA(K) || any(K<=first)) stop("Invalid first/K histories.")',
    '',
    '# Chronological age at the ORIGIN of each annual movement interval.',
    '# Cub first capture y -> birth Q1 y; yearling first capture y -> birth Q1 y-1.',
    'birth_year <- as.integer(eligible$birth_year)',
    'age_origin <- matrix(NA_real_,nind,n_prim)',
    'age_c <- matrix(0,nind,n_prim)',
    'for(i in seq_len(nind)) for(k in (first[i]+1L):K[i]){',
    '  age_origin[i,k] <- years[k-1L]-birth_year[i]',
    '  age_c[i,k] <- age_origin[i,k]-AGE_CENTER',
    '}',
    'age_active <- unlist(lapply(seq_len(nind),function(i) age_origin[i,(first[i]+1L):K[i]]))',
    'if(any(!is.finite(age_active)) || any(age_active<0)) stop("Invalid reconstructed chronological ages.")',
    'age_init <- vapply(seq_len(nind),function(i) age_c[i,first[i]+1L],numeric(1))',
    'cat("  known-age subset:",nind,"badgers | origin-age range:",min(age_active),"to",max(age_active),"years\\n")'
  ),collapse="\n"),
  "chronological age construction"
)

# ---- remove adult-entry effect: no adult entrants have known exact age -------
...
txt <- replace_txt_once(
  ...
  '    p_disp_init[i] <- ilogit(alpha_disp_init+beta_disp_init_sex*sex[i]+beta_age*age_init[i])',
  "initial-state age effect"
)
...
txt <- replace_txt_once(
  ...
  paste(c(
    '  beta_RD_sex ~ dnorm(0,sd=1)',
    '  beta_age ~ dnorm(0,sd=.35)',
    '  OR_age_per_year <- exp(beta_age)'
  ),collapse="\n"),
  "age prior"
)
...
txt <- replace_txt_once(
  ...
  '               sex=as.integer(sex_data),age_c=age_c,age_init=as.numeric(age_init),',
  "age constants"
)
...
txt <- replace_txt_once(
  ...
  '  alpha_disp_init=v6_post_mean("alpha_disp_init",-3.29),beta_age=0,',
  "age warm start"
)
...
txt <- replace_txt_once(
  ...
  '       beta_age=warm$beta_age+rnorm(1,0,.03),\n       beta_disp_init_sex=warm$beta_disp_init_sex+rnorm(1,0,.05),',
  "age initial value"
)
...
txt <- replace_txt_once(
  ...
  '  "alpha_disp_init","beta_disp_init_sex","beta_age","OR_age_per_year",',
  "age monitor"
)
...
txt <- replace_txt_once(
  ...
  'RD_global <- c("alpha_RD","beta_RD_sex","beta_age")',
  "age sampler block"
)
...
txt <- replace_txt_once(
  ...
  'init_global <- c("alpha_disp_init","beta_disp_init_sex")',
  "initial-state sampler block"
)

# Save age with every movement interval for direct interpretation.
txt <- replace_txt_once(
  txt,
  '         is_initial_interval=ks==(first[i]+1L),\n         node=paste0("disp[",i,", ",ks,"]"))',
  '         is_initial_interval=ks==(first[i]+1L),age_years=age_origin[i,ks],\n         node=paste0("disp[",i,", ",ks,"]"))',
  "age in interval index"
)

txt <- replace_txt_once(
  txt,
  '    "alpha_RD","beta_RD_sex","alpha_DD","beta_DD_sex",',
  '    "alpha_RD","beta_RD_sex","beta_age","OR_age_per_year","alpha_DD","beta_DD_sex",',
  "age quick summary"
)
...
txt <- gsub("all 1,756 badgers that can contribute to the directional V7 analyses",
            "all 1,756 movement-informative badgers with reconstructable chronological age",txt,fixed=TRUE)
txt <- gsub("exact directional-analysis population from the all-badger audit",
            "maximum known-age movement population from the inclusive population audit",txt,fixed=TRUE)
txt <- gsub('model="V7M_movement_only_modular_source"','model="V8_movement_chronological_age"',txt,fixed=TRUE)
txt <- gsub('sample_source="all_directional_badgers_from_V7_audit"',
            'sample_source="movement_badgers_first_caught_as_cub_or_yearling"',txt,fixed=TRUE)
txt <- gsub('adult_entry_effect="initial_state_probability_only"',
            'age_effect="chronological age in years; shared slope for initial high-mobility probability and later local-to-high transitions"',txt,fixed=TRUE)
txt <- gsub('audit_file=AUDIT_FILE',
            'population_definition="results/V7_population_inclusion_audit.rds",birth_rule="Cub: Q1 first-capture year; Yearling: Q1 previous year",age_center_years=AGE_CENTER',txt,fixed=TRUE)

# Add wrapper constants to generated source.
