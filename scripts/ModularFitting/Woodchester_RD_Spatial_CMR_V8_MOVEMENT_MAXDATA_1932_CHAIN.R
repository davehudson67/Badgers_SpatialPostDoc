...
replace_block <- function(x,start_pattern,end_pattern,replacement,label){
  ...
  if(length(s)!=1L || length(e)!=1L || e<s)
    stop("Could not uniquely patch ",label," in frozen base model.")
...
replace_once <- function(x,old,new,label){
  ...
  x[hit] <- sub(old,new,x[hit],fixed=TRUE)
  x
}
...
src <- replace_block(
  ...
  c("required_files <- c(encounter_file,individual_file,sett_file,spatial_file,V6C_RESULT_FILE)"),
  "required file list"
)
...
src <- replace_once(
  src,
  "Directional population should have known sex for every badger.",
  "Maximal movement population should have known sex for every badger under the current audited snapshot.",
  "sex assertion text"
)

# ---- metadata/output names ---------------------------------------------------
src <- gsub("1285","1932",src,fixed=TRUE)
src <- gsub("all 1,932 badgers that can contribute to the directional V7 analyses",
            "all 1,932 badgers with at least two spatially usable live years",src,fixed=TRUE)
src <- gsub("exact directional-analysis population from the all-badger audit",
            "maximum movement-informative population from the inclusive population audit",src,fixed=TRUE)
src <- gsub("all_directional_badgers_from_V7_audit",
            "all_badgers_with_at_least_2_usable_spatial_live_years",src,fixed=TRUE)
src <- gsub("audit_file=AUDIT_FILE",
            "population_definition=\"results/V7_population_inclusion_audit.rds\"",src,fixed=TRUE)

# The wrapper constant is used by the generated source population assertion.
src <- append(src,values="",after=0L)
src <- append(src,values=paste0("EXPECTED_N <- ",EXPECTED_N,"L"),after=0L)
